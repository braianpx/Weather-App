...
const addAndRealoadFav = () => {
    ...
    setTimeout(()=>{dispatch(getFavorites())},100)
        ...
        alert('"you can no longer add more cities to favorites, you reached the maximum limit"')
    }
}
const removeCity = () => {
    ...
    return(
        ...
        <div id={fadingRemove?"idCardFading":"idCard"} class="card m-5" >
            <div class="d-flex w-100 justify-content-end my-0 py-0">
            <i id={searchCity(props.name)?"idRemoveFavorite":"idAddFavorite"} class="bi bi-suit-heart-fill fs-4 me-3 text-secondary" onClick={()=>user.username?addFav()?dispatch(removeFavorites(props.name)):addAndRealoadFav():alert('you must first log in to be able to add a city to favorites')}></i>
            {
            props.switchx?
                <i id="idRemoveCard" class="bi bi-x-square my-0 py-0 fs-4 me-2 text-secondary" onClick={()=> removeCity()}></i>
                :
                null
            }
            </div>
            <div style={{"z-index":"-1 !important"}} id="carouselExampleDark" class="card-body w-100 h-50 text-center text-white row row-cols-2 align-content-end mt-0 p-1" >
                <div class="p-0 ms-1">
