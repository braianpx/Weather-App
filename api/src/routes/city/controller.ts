...
try{
    const name:string = String(names);
if(name){
    let nameWithoutSpace:string = name.replace(" ","%20")
    const city = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${nameWithoutSpace}&appid=${API_KEY}`);
...
try{
    ...
    const cities = names.map(async(el)=>{
        return axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${el}&appid=${API_KEY}`);
    });
